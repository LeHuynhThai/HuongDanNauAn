...
extension DatabaseManager {
    ...
    func addRecipe(name: String,
        ...
        do {
            ...
            let insert = recipes.insert(
                ...
                self.recipeCreatedAt <- getCurrentDateString()
            )
            try db?.run(insert)
            return true
        } catch {
        ...
        do {
            for row in try db!.prepare(recipes.filter(self.recipeUserId == userId)) {
                ...
                let recipeObj = Recipe(
                    ...
                    createdAt: parseDate(dateString: row[recipeCreatedAt]),   // FIXED
        ...
        do {
            for row in try db!.prepare(recipes) {
                ...
                let recipeObj = Recipe(
                    ...
                    ingredients: ingredients,
                    instructions: instructions,
                    createdAt: parseDate(dateString: row[recipeCreatedAt]),
                    cookTime: row[recipeCookTime],
                    difficulty: difficulty,
                    imageURL: row[recipeImageURL]
                )
    ...
    func decodeStringArray(_ jsonString: String) -> [String] {   // ← không để private
